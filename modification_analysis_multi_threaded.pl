...
get_all_mature_mirnas_align_indexes();
...
my $mirmod_debug_output = "$outputFolder/out_mirmod.txt";
open (MIRM_OUT,">$mirmod_debug_output");

print MIRM_OUT "@filenames\n";
print MIRM_OUT "$outputFolder\n";
print MIRM_OUT "$curTaxonName\n";
close(MIRM_OUT);
...
foreach my $filename(@filenames){
    ...
    push @mirmod_per_file_threads, threads->create(sub{
        ...
        while(<PROC>){
            ...
            if (!$fail){
                ...
                print "%%%%%% five_nt = $five_nt, three_nt = $three_nt %%%%%%\n";
...
foreach(@mirmod_per_file_threads){
    $_->join();
}
...
sub get_all_mature_mirnas_align_indexes{
